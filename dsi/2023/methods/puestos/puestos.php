...
class puesto {
		...
		try {
			...
			foreach ($fetchAll as $result) {
				...
				$dato.= '<tr>
					...
					<td>'.htmlspecialchars($result['nombre_puesto_dsi']).'</td>
					<td>
				        <div class="btn-group" role="group" aria-label="Basic example">
				          <a type="button" class="btn btn-warning" href="editPuesto?id='.$result['idpuesto_dsi'].'">Editar</a>
				          <a type="button" class="btn btn-danger" onclick="eliminarPuesto('.$result['idpuesto_dsi'].');">Eliminar</a>
				        </div>
				      </td>
				</tr>';
			}
	...
	public function datosPuesto($id) {
		try {
			$sql = 'SELECT * FROM puesto_dsi WHERE idpuesto_dsi=:id LIMIT 1';
			$query = $this->conexion->prepare($sql);
			...
			foreach ($fetchAll as $result) {
				$datos = array(0 => $result['idpuesto_dsi'], 
							   1 => $result['nombre_puesto_dsi']
								);
			}
			return $datos;
		} catch (Exception $e) {
	...
	public function editarPuesto() {
		try {
			$validacion = self::validaDataEditPuesto($_POST['puestoName']);
			if ($validacion === true) {
				$id = $_POST['idPuesto'];
				$nombre = $_POST['puestoName'];
				$sql = 'UPDATE puesto_dsi SET nombre_puesto_dsi = :nombre WHERE idpuesto_dsi = :id';
				$query = $this->conexion->prepare($sql);
	...
	public function AddPuesto() {
		try {
			...
			$sql = 'INSERT INTO puesto_dsi (nombre_puesto_dsi) VALUES (:nombre)';
			$query = $this->conexion->prepare($sql);
			$query->bindParam(':nombre', $nombre, PDO::PARAM_STR);
			if ($query->execute()) {
				return true;
