...
$Aniversario = new actividad();
$aniversario="Aniversario";
$ver = $Aniversario->ExtraerActividadesPaginacion($aniversario,$limite);
if(isset($_SESSION['tipo'])&& $_SESSION['tipo']=="Administrador")
{
	...
	if(is_array($ver))
	{
		...
		?>
		<br><br>
		<button type="button" class="btn btn-lg btn-success" title="Generar Pdf" id="generarpdfaniversarios">Generar Pdf <span class="glyphicon glyphicon-cloud-download"></span></button>
		<br><br>
		...
		<ul class="pager">
			...
			if($limite>0)
			{
				$limit = $limite-12;
				?>
					<li class="previous"><a class="text-info" id="atrasactividad" onclick="cargaractividad(<?=$limit;?>,'aniversarios.php')" href="#">&larr; Atras</a></li>
					<?php
			}
			...
			if($limite<$ver[0]['cantidad']-12)
			{
				$limit = $limite+12;
				?>
				<li class="next"><a class="text-info" id="siguientesactividad" onclick="cargaractividad(<?=$limit;?>,'aniversarios.php')" href="#">Siguientes &rarr;</a></li>
				<?php
			}
...
<tr>
	...
	<td><?= $key['telefono'];?></td>
	<td><?= $key['hora'];?></td>
	<td> <button type="button" class="btn btn-lg btn-success" id="verdatos">Ver <span class="glyphicon glyphicon-ok-sign"></span></button></td>
</tr>
<?php }?>
</table>
...
{
	if($ver==0){
		echo "<h3>Error no exites aniverarios registrada en el sistema</h3>";
	}else if($ver==1){
		echo "<h3>Error al obtener los datos</h3>";
	}
}
